...
import { useDeleteComboOffers, useGetComboOffers } from 'restaurants/combo-offers/hooks';
...
const ComboOffersList: React.FC = () => {
  ...
  const filteredCombos = useMemo(
    () => filters?.query
      ? data?.data?.combos?.filter(
          (combo) => combo?.comboName?.toLowerCase()?.includes(filters?.query?.toLowerCase() ?? '')
        )
      : data?.data?.combos,
    [data, filters?.query]
  );
      ...
      actions: (
        <div className="table-actions-cell">
          ...
          <button
            type="button"
            className="action-button"
            onClick={() => navigate(`${routes.details}/${combo.comboId}/${routes.edit}`)}
          >
            <Icon name="pencil" />
          </button>
        </div>
    ...
    <>
      ...
      {selectedCombo && (
        <ComboDeleteModal
          ...
          onDelete={() => mutateAsync(selectedCombo?.comboId).then(() => setSelectedCombo(undefined))}
          loading={isLoading}
        />
      )}
    </>
