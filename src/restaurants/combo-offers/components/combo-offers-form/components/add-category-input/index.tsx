...
const ItemSubForm: React.FC<{
  ...
  const restaurantId = Form.useWatch(["comboCategories", `${categoryIndex}`, "items", `${itemIndex}`, "restaurantId"], form);
  ...
  >
    {(fields, { add, remove }) => (
      <>
        ...
        {!disabled && (
          <TableForm.Item
            label={`Combo Category ${(fields?.length ?? 0) + 1}`}
          >
            <Form.Item>
              <Button
                size="large"
